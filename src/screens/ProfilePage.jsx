...
import { DELETE_WRITER } from '../constants/API'
...
const ProfilePage = ({ route }) => {
    ...
    const deleteRequestToApi = ()=>{
        axios.get(DELETE_WRITER + globalId)
        .then(response => {
        ...
        <ScrollView className="flex w-full">
            ...
            <View className="w-full h-auto flex flex-col pt-6 px-6 mb-12">
                ...
                <Text className="text-[14px] font-medium text-[#505050] mt-1">
                    {globalUsername}  •  <Text className='text-[#13AB3E]'>Writer</Text> 
                </Text>
            </View>
            <View className="w-full h-full px-6 py-0 flex flex-col">
                <TouchableOpacity className="w-full h-[66px] border-[1.5px] border-[#9B3434] rounded-custom flex flex-row justify-between items-center mb-6" onPress={()=>{navigation.navigate('MyUploadsPage')}}>
                ...
                <TouchableOpacity className="w-full h-[66px] border-[1.5px] border-[#9B3434] rounded-custom flex flex-row justify-between items-center mb-6" onPress={()=>{navigation.navigate('AddNewBook')}}>
                    <Text className="text-[18px] font-medium text-[#9B3434] pl-6">
                        Add New Book
                    </Text>
                    <View className="mr-6">
